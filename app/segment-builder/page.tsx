...
import { Trash2, Users, Send, History, Zap, DollarSign, Calendar, MapPin, ShoppingBag, UserCheck, Grip } from 'lucide-react';
...
const fetchPreviewAPI = async (rules:Rule[]) => {
  try {
    const response = await fetch("http://localhost:8080/api/segments/preview", {
      method: "POST",
...
const CampaignBuilder = () => {
  ...
  const [audienceData, setAudienceData] = useState({ count: 0, demographics: null, loading: false });
  ...
  const [campaigns] = useState([
    ...
    },
    {
      ...
      status: 'completed',
      rules: [
        { field: 'last_order_date', operator: 'older_than', value: 60 }
      ]
    },
    {
      id: 3,
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        setAudienceData({ count: 0, demographics: null, loading: false, error: 'Failed to load preview' });
      }
    }, 500);
  ...
  const handleDragStart = (e:React.DragEvent, item:DraggedItem, type:DraggedItem["type"]) => {
  ...
  const handleDrop = (e:React.DragEvent, index:number) => {
    ...
    if (draggedItem.type === 'rule') {
      const newRule = {
        ...
        operator: draggedItem.operators[0],
      ...
      const templateRules = draggedItem.rules.map((rule, i) => ({
        ...
        logic: i < draggedItem.rules.length - 1 ? 'AND' : undefined
      }));
  ...
  const addLogicConnector = (index, logic) => {
  ...
  try {
    ...
    const response = await fetch('http://localhost:8080/api/segments/save', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        name: campaignName,
        description: `Segment with ${rules.length} rule${rules.length > 1 ? 's' : ''}`,
        rules: rules
      })
    });
    ...
    alert(`Segment "${campaignName}" saved successfully with ${result.segment.audience_size} customers!`);
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-3">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Rule Blocks</h2>
              ...
              <div className="mt-6">
                <h3 className="text-md font-semibold text-gray-900 mb-3">Logic Connectors</h3>
                <div className="space-y-2">
                  <div
                    draggable
                    onDragStart={(e) => handleDragStart(e, { operator: 'AND' }, 'logic')}
                  ...
                  >
                    <div className="flex items-center justify-center">
                      <span className="text-sm font-medium text-green-800">AND</span>
                      <Grip className="w-4 h-4 ml-auto opacity-40" />
                    </div>
                  </div>
                  <div
                    draggable
                    onDragStart={(e) => handleDragStart(e, { operator: 'OR' }, 'logic')}
          ...
          <div className="col-span-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-2">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Audience Rules</h2>
              ...
              >
                {rules.length === 0 ? (
                  <div className="text-center text-gray-500 py-20">
                    ...
                    <p className="text-sm">Drag rule blocks here to start creating your segment</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {rules.map((rule, index) => {
                      const ruleType = ruleTypes.find(type => type.field === rule.field);
                        ...
                        <div key={rule.id}>
                          ...
                          <div
                            className={`h-2 rounded transition-colors ${
                              dragOverIndex === index ? 'bg-blue-200' : 'transparent'
                            }`}
                          ...
                          {/* Rule block */}
                          <div className={`${ruleType?.color || 'bg-gray-100'} p-2 rounded-lg border-2`}>
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1 flex items-center gap-3">
                                ...
                                <input
                                  type={ruleType?.valueType === 'number' ? 'number' : ruleType?.valueType === 'date' ? 'date' : 'text'}
                                  value={rule.value}
                          ...
                          {index < rules.length - 1 && (
                            <div className="flex items-center justify-center py-2">
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={() => addLogicConnector(index, 'AND')}
                    ...
                    <div
                      className={`h-8 rounded transition-colors ${
                        dragOverIndex === rules.length ? 'bg-blue-200' : 'border-2 border-dashed border-gray-200'
                      }`}
                      onDragOver={(e) => handleDragOver(e, rules.length)}
                      onDrop={(e) => handleDrop(e, rules.length)}
                    />
          ...
          <div className="col-span-3">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Audience Preview</h2>
              ...
              {audienceData.error && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                  <div className="text-red-800 text-sm">{audienceData.error}</div>
                </div>
              )}
              ...
              {audienceData.demographics && !audienceData.loading && (
                <div className="space-y-4">
                  <div>
                    <h4 className="text-sm font-medium text-gray-700 mb-2">By City</h4>
                    <div className="space-y-1">
                      {Object.entries(audienceData.demographics.by_city).map(([city, count]) => (
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-gray-700 mb-2">By Spending</h4>
                    <div className="space-y-1">
                      {Object.entries(audienceData.demographics.by_spending_tier).map(([tier, count]) => (
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-gray-700 mb-2">By Activity</h4>
                    <div className="space-y-1">
                      {Object.entries(audienceData.demographics.by_recency).map(([period, count]) => (
